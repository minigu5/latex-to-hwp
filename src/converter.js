...
(function (global) {
  ...
  function convertCmd(name, tokens, pos) {
    ...
    if (name === 'boxed') {
      var arg = readArg(tokens, pos);
      return { text: '{BOX{' + arg + '}}', keyword: false };
    }
  ...
  // 들어오면 잃어버린 별표를 복원해 `N^{*}(t)` 로 해석한다.
  //
  // 다만 `e^(x+y)`, `2^(10)`, `f^(n)(x)` (n계도함수) 처럼 `^(...)` 가 정당하게
  // 등장하는 비형식 표기도 있어, 무차별 적용은 위험하다. 그래서 괄호 안이 영문자
  // 1~2글자(예: `(t)`, `(R)`, `(ij)`)일 때만 별표 손실로 본다. 숫자(`(10)`)나
  // 연산자(`(x+y)`)를 포함하면 손대지 않는다.
  // 한계: `f^(n)(x)` 처럼 1자 변수가 미분 인덱스로 쓰인 경우는 별표 손실과
  // 구분이 불가능하므로 함께 잡힌다.
  function recoverLostStarSuperscript(s) {
    return s.replace(/\^\(([a-zA-Z]{1,2})\)/g, '^{*}($1)');
  }
