...
| `\left( ... \right)` | **정책 결정 필요** (아래 4절) | LEFT( ... RIGHT) 권장 vs 단순 `( )` |
...
## 4. `\left` / `\right` 정책 (확인 필요)

- **명세 권장 (2.5)**: 분수·합 등 큰 내용을 괄호로 감쌀 때는 `LEFT( ... RIGHT)`를 쓴다. 안 쓰면 괄호 크기가 내용에 맞춰 커지지 않아 "보기에 좋지 않은 수식"이 된다.
- **골든 예시**: `\left( \frac{...}{...} ... \right)`를 단순 `( ... )`로만 변환했다.

→ 두 방식이 충돌한다. 현재 정책: **골든 예시 재현을 위해 기본은 `( )`로 변환**하되, 향후 사용자 확인 후 "분수/합/적분 등 높이가 큰 내용을 감싼 경우 `LEFT(`/`RIGHT)`로" 옵션화 검토. (TODO: 사용자에게 한글 실제 렌더링 확인)
...
- `\left`/`\right` 처리 정책 (4절) — 한글 실제 렌더링 확인 후 확정.
