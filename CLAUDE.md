...
- **렌더링된 수식 붙여넣기 감지 및 Fallback:** 사용자가 AI 챗봇에서 렌더링된 수식을 그대로 복사하면 위/아래 첨자 구조가 손실된다. `app.js` 에서는 이를 휴리스틱으로 감지해 경고 모달을 띄우고, ChatGPT와 Google AI Studio 포맷을 구분하여 최소한의 기호 매핑을 시도하는 Fallback 로직을 제공한다.
